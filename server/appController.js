...
// API endpoint to GET count of recipes liked per user level
router.get('/recipes-liked-per-user-level', async (req, res) => {
    try {
        const data = await appService.fetchRecipesLikedPerUserLevel();
        if (data.length === 0) {
